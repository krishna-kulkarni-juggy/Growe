...
`,
  ...
  const downloadLeaseAgreement = (lease) => {
    try {
      // Create a formatted document content
      const documentContent = `
${lease.lease_agreement.document_name}

${lease.lease_agreement.document_content}

LEASE SUMMARY:
=============

KEY TERMS:
${lease.lease_agreement.summary.key_terms.map((term, index) => `${index + 1}. ${term}`).join('\n')}

FINANCIAL SUMMARY:
- Total Annual Cost: $${lease.lease_agreement.summary.financial_summary.total_annual_cost.toLocaleString()}
- Escalation Rate: ${lease.lease_agreement.summary.financial_summary.escalation_rate}
- Security Deposit: $${lease.lease_agreement.summary.financial_summary.deposit_amount.toLocaleString()}
- Additional Fees: ${lease.lease_agreement.summary.financial_summary.additional_fees.join(', ')}

ACTION ITEMS:
${lease.lease_agreement.summary.action_items.map((item, index) => 
  `${index + 1}. [${item.priority.toUpperCase()} PRIORITY] ${item.description} (Due: ${new Date(item.due_date).toLocaleDateString()}) - Status: ${item.status.toUpperCase()}`
).join('\n')}

Generated by Growe Lease Administration System
Date: ${new Date().toLocaleDateString()}
      ...
      a.href = url;
      a.download = lease.lease_agreement.document_name.replace('.pdf', '.txt');
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      
      toast.success(`Downloaded ${lease.lease_agreement.document_name}`);
    } catch (error) {
  ...
  const printLeaseAgreement = (lease) => {
    try {
      const printContent = `
        <html>
          <head>
            <title>${lease.lease_agreement.document_name}</title>
            <style>
              body { font-family: Arial, sans-serif; margin: 20px; line-height: 1.6; }
              h1 { color: #2563eb; border-bottom: 2px solid #2563eb; padding-bottom: 10px; }
              h2 { color: #374151; margin-top: 30px; }
              .section { margin: 20px 0; }
              .action-item { margin: 10px 0; padding: 10px; background: #f9fafb; border-left: 4px solid #2563eb; }
              .priority-high { border-left-color: #dc2626; }
              .priority-critical { border-left-color: #991b1b; }
              .priority-medium { border-left-color: #d97706; }
              .priority-low { border-left-color: #2563eb; }
            </style>
          </head>
          <body>
            <h1>${lease.lease_agreement.document_name}</h1>
            <div class="section">
              <pre style="white-space: pre-wrap; font-family: Arial, sans-serif;">${lease.lease_agreement.document_content}</pre>
            </div>
            <div class="section">
              <h2>Lease Summary</h2>
              <h3>Key Terms:</h3>
              <ul>
                ${lease.lease_agreement.summary.key_terms.map(term => `<li>${term}</li>`).join('')}
              </ul>
              <h3>Financial Summary:</h3>
              <ul>
                <li>Total Annual Cost: $${lease.lease_agreement.summary.financial_summary.total_annual_cost.toLocaleString()}</li>
                <li>Escalation Rate: ${lease.lease_agreement.summary.financial_summary.escalation_rate}</li>
                <li>Security Deposit: $${lease.lease_agreement.summary.financial_summary.deposit_amount.toLocaleString()}</li>
                <li>Additional Fees: ${lease.lease_agreement.summary.financial_summary.additional_fees.join(', ')}</li>
              </ul>
              <h3>Action Items:</h3>
              ${lease.lease_agreement.summary.action_items.map(item => `
                <div class="action-item priority-${item.priority}">
                  <strong>[${item.priority.toUpperCase()} PRIORITY]</strong> ${item.description}<br>
                  <small>Due: ${new Date(item.due_date).toLocaleDateString()} | Status: ${item.status.toUpperCase()}</small>
                </div>
              `).join('')}
            </div>
            <div class="section">
              <p><small>Generated by Growe Lease Administration System on ${new Date().toLocaleDateString()}</small></p>
            </div>
          </body>
        </html>
      `;
      ...
      toast.error('Failed to print lease agreement');
